...
void
drawsplitScreen(Player *p1, Player *p2, std::vector<Enemy *> enemies, int width, int height,
                const Uint8 *state,
...
//        GAMESCREEN PLAYER 1
        ...
        if (!p1->isDead())
            c1->move();
        drawsplitScreen(p1, p2, enemies, width, height, state, params, idBulletTexture, idDesert, idSolSkybox,
                        arbres,
